<template>
	<div id="container">
			...
			<el-table-column label="操作" width="">
				<template slot-scope="scope">
					...
					<el-button @click="downloadRaw(scope.row,1)" size="mini" type="success" plain icon="el-icon-delete">
						下载原文件</el-button>
					<el-button  @click="downloadRaw(scope.row,0)" size="mini" type="success"
		...
		<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
			:current-page="queryParams.pageNum" :page-sizes="[20, 40, 80, 160]" :page-size="queryParams.pageSize"
...
<script>
	...
	import PlantQuery from './txts/components/PlantQuery'
	import PlantOperation from './txts/components/PlantOperation'
	import PlantAddDialog from './txts/components/PlantAddDialog'
	import PlantReviseDialog from './txts/components/PlantReviseDialog'
	import PlantDetailDialog from './txts/components/PlantDetailDialog'
	export default {
		data() {
			return {
				// table选中数据
			...
			downloadRaw(tar, isRaw) {
				this.$axios({
					...
					.then(res => {
						...
						link.href = url;
						link.setAttribute('download', `${tar.nativename}${isRaw==0? 'treated':''}.${tar.fileType}`);
